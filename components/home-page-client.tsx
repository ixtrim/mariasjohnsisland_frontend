...
import {
  FloatingDecorations, SectionDivider, DecorativeDivider,
  ChiliPepper, Lime, Cactus, Margarita,
} from "@/components/mexican-decorations";
import type { HomePageFields, GlobalOptionsFields } from "@/lib/cms";

if (typeof window !== "undefined") {
  gsap.registerPlugin(ScrollTrigger);
}

// ─── Gallery images (static — managed via WP Media if needed later) ──────────
...
export default function HomePageClient({ cms, options }: HomePageClientProps) {
  ...
  const welcomeText1 = cms.welcome_text_1 || DEFAULT_WELCOME_TEXT_1;
  const welcomeText2 = cms.welcome_text_2 || DEFAULT_WELCOME_TEXT_2;
  const addressLine1 = options.footer_address_1 || "2817 Suite 6 Maybank Hwy";
  const addressLine2 = options.footer_address_2 || "Johns Island, SC 29455";
  const phone        = options.footer_phone     || "(843) 557-1005";
  const orderUrl     = options.order_online_url || "https://www.toasttab.com/marias-mexican-grill-2817-maybank-hwy-ste-7/v3";

  const infoHours = options.footer_hours?.length
    ? options.footer_hours.slice(0, 3)
    : [
  ...
  useEffect(() => {
    if (welcomeRef.current) {
      ...
      gsap.to(elements, {
        ...
        scrollTrigger: { trigger: welcomeRef.current, start: "top 80%", toggleActions: "play none none none" },
      });
    }
    ...
    if (menuRef.current) {
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          gsap.to([decorLeft, decorRight], { opacity: 1, scale: 1, rotation: 0, duration: 0.8, stagger: 0.15, ease: "back.out(1.7)", delay: 0.5 });
        },
      });
    }
    ...
    <>
      ...
      <HeroSection
        ...
        ctaButtons={[
          ...
          { label: "Order Online", href: orderUrl, variant: "outline", external: true },
        ]}
      />
          ...
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            <div className="menu-image relative">
              <div className="relative aspect-[4/5] rounded-2xl overflow-hidden shadow-2xl transform hover:scale-[1.02] transition-transform duration-500">
                <Image src="/images/dishes_18.jpg" alt="Restaurant Interior" fill className="object-cover" sizes="(max-width: 1024px) 100vw, 50vw" />
            ...
            <div className="menu-content">
              <span className="text-[#00a9e4] text-sm uppercase tracking-[0.2em] font-medium">Discover</span>
              ...
              <div className="flex flex-wrap gap-4">
                <Link href="/menu" className="btn-animated inline-flex items-center gap-2 px-8 py-4 bg-[#00a9e4] text-white rounded-[5px] font-semibold text-sm uppercase tracking-wide shadow-lg shadow-[#00a9e4]/30">
                  View Full Menu <ArrowRight className="w-4 h-4" />
                </Link>
                <Link href="/happy-hour" className="btn-animated inline-flex items-center gap-2 px-8 py-4 border-2 border-[#00a9e4] text-[#00a9e4] hover:bg-[#00a9e4] hover:text-white rounded-[5px] font-semibold text-sm uppercase tracking-wide transition-colors duration-300">
                  Happy Hour
                </Link>
              </div>
            </div>
      ...
      <section className="py-20 bg-white relative">
        <div className="container mx-auto px-4">
          <SectionHeading eyebrow="Our Dishes" title="A Taste of Jalisco" description="Fresh ingredients, bold flavors, and traditional recipes crafted with passion." />
        ...
        <div ref={infoRef} className="container mx-auto px-4 relative z-10">
          <div className="grid md:grid-cols-3 gap-10 text-center">
            <div className="info-card flex flex-col items-center p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-[#00a9e4]/50 transition-all duration-500 hover:transform hover:scale-105">
              ...
              <p className="text-white/70">{addressLine1}</p>
              <p className="text-white/70">{addressLine2}</p>
            </div>
            <div className="info-card flex flex-col items-center p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-[#00a9e4]/50 transition-all duration-500 hover:transform hover:scale-105">
              <div className="w-20 h-20 rounded-[5px] bg-[#00a9e4]/20 flex items-center justify-center mb-6 pulse-glow">
              ...
              {infoHours.map((row, i) => (
                <p key={i} className="text-white/70">{row.day}: {row.hours}</p>
              ))}
            </div>
            <div className="info-card flex flex-col items-center p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-[#00a9e4]/50 transition-all duration-500 hover:transform hover:scale-105">
              <div className="w-20 h-20 rounded-[5px] bg-[#00a9e4]/20 flex items-center justify-center mb-6 pulse-glow">
              ...
              <a href={`tel:+1${phone.replace(/\D/g, "")}`} className="text-[#00a9e4] hover:text-[#c1eaf9] transition-colors text-lg">
                {phone}
              </a>
            </div>
          </div>
      ...
      <section className="py-24 bg-[#c1eaf9] relative">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div ref={ctaRef} className="max-w-4xl mx-auto text-center bg-white rounded-3xl shadow-2xl p-10 md:p-16 relative overflow-hidden">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <Link href="/location" className="btn-animated inline-flex items-center gap-2 px-10 py-5 bg-[#121212] text-white rounded-[5px] font-semibold text-sm uppercase tracking-wide shadow-xl shadow-[#121212]/30">
